...
avg_rating_genre= list(collection.aggregate(pipeline))

#print the reults
for entry in avg_rating_genre:
    print(f"Genre: {entry['_id']}, Average Rating: {entry['average_rating']:.2f}")


with open("average_rating_by_genre.json", "w") as outfile:
    json.dump(avg_rating_genre, outfile, indent=4)